...
- canonical acoustic ID reconciliation: current canonical `SS_*`/`Q_*` outputs are separated from explicit `PROVISIONAL_*` compatibility measurements
- unresolved semantic result creation through privileged RPCs
- executable Evidence Engine v2 over immutable MeasurementRecords with canonical `EV_*` marker IDs and composite structural rules
- immutable `evidence_ledgers` persistence through a service-only RPC
- executable Dimension Engine v2 over immutable Evidence Ledgers with Canon-defined structural Evidence-to-Dimension requirements
- immutable `dimension_results` persistence through a service-only RPC
- Dimension Calibration Foundation with structural mapping recognized and numeric scoring still blocked by `CALIBRATION_REQUIRED`
...
- hosted staging credentialed pipeline validation
...
Evidence Engine v2 consumes immutable `measurement_records` only. It never reads WAVs, reruns acoustic extraction, mutates measurements, or creates downstream semantic inference.

The v2 engine produces a canonical structural Evidence Ledger:

- marker IDs are the authoritative `EV_*` IDs from the Evidence Marker Registry v0.1
- composite markers preserve accepted, rejected, missing, and insufficient source components
...
- every evidence entry records source measurement identity, prompt scope, quality, provenance, rule version, and active evidence versions
...
Legacy `evidence_records` rows remain immutable historical compatibility data. The active canonical write path for MeasurementRecord-to-Evidence processing is `evidence_ledgers`; result manifests now have a canonical `result_manifest_evidence_ledgers` membership table while the older `result_manifest_evidence` table remains for legacy rows.

## Dimension Engine v2

Dimension Engine v2 consumes immutable `evidence_ledgers` only. It never reads WAVs, reruns acoustic extraction, reads MeasurementRecords directly, mutates upstream records, or creates downstream State, Constellation, Pattern, Narrative, or Resonance output.
...
Canonical structural Evidence families, candidate markers, required markers, prompt prerequisites, independent-family counts, and D3 abstentions are encoded for all 16 Dimensions. Calibrated Dimension scoring, weights, normalization, posterior construction, and confidence formulas remain deferred. Dimension Engine v2 therefore persists deterministic unresolved/abstained Dimension Results with posterior values and confidence set to `null`. Missing, rejected, unavailable, insufficient, supported, and contradicted Evidence entries remain distinct in qualification/provenance and are never converted to zero.
...
- Evidence-to-Dimension family requirements where Canon defines them
- candidate Evidence marker IDs for all 16 Dimensions
- required Evidence markers where Canon defines them
- independent Evidence-family counting
- D3 abstention rules

Not scientifically defined:

- calibrated Evidence-to-Dimension scoring mappings
- directionality
- weights
- thresholds
- normalization
